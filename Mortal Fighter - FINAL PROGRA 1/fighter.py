...
class Fighter():
  ...
  def __init__(self, jugador, x, y, flip, data, sprite_sheet, animation_steps, sonido):
    ...
    self.running = False #si el jugador esta corriendo
    self.jump = False #si el jugador esta en el aire
  ...
  def cargar_imagenes(self, sprite_sheet, animation_steps):
    ...
    -Crea una lista temporal para los frames de la animacion actual
    ...
    -Escala la imagen del frame segun el factor de escala y la agrega a la lista temporal
  ...
  def moverse(self, screen_width,screen_height, surface, target, round_over):
    ...
    #reduce el cooldown de ataque si esta activo
  ...
  def update(self):
    '''
    Descripcion: Actualiza la animacion y estado del luchador en funcion de su accion actual.
    Detalles: Determina la animacion en curso en funcion de las condiciones (salud, ataques, daño, etc.).
    ...
    if self.frame_index >= len(self.animation_list[self.action]):
      if self.alive == False:
        ...
        return True  # Indica que la animación de muerte ha terminado
      else:
        self.frame_index = 0
  ...
  def attack(self, target):
    '''
    Descripcion: Ejecuta un ataque contra el luchador objetivo.
    Arg:
  ...
  def update_action(self, accion_nueva):
    '''
    Descripcion: Cambia la accion del luchador y actualiza el estado de la animacion.
    Arg:
